# CellGuard.AI — Streamlit Dashboard v3 (Enhanced UI + Smart Column Mapping)
...
def main():
    ...
    st.sidebar.markdown("---")
    st.sidebar.markdown("### CSV format\n`voltage, current, temperature, soc, cycle` (names can vary).")
    ...
    label, _ = pack_health_label(avg_score)
    ...
    with tab_overview:
        ...
        with left:
            ...
            fig_health.update_layout(margin=dict(l=10, r=10, t=40, b=10))
    ...
    with tab_anomaly:
        ...
        with st.expander("ℹ️ How anomalies are found"):
            st.markdown(
                "- Uses **Isolation Forest** on voltage, current, temperature & trends.\n"
                "- Flags unusual behaviour that may indicate failing cells.\n"
                "- Combine with health score & temperature for maintenance decisions."
            )
    ...
    with tab_table:
        ...
        with st.expander("ℹ️ Tips for using this CSV"):
            st.markdown(
                ...
                "- Rolling features help in advanced models.\n"
                "- Ideal for EV fleet analytics & lifetime tracking."
            )
